...
const View = ({view}) => {
    ...
    const { temp, feels_like, humidity, pressure, clouds, dew_point, 
        ...
        const newPressure = Math.round(pressure / 1.333),
            ...
            newSunrise = moment.unix(sunrise).utc().add(timezone_offset, 'seconds').format('LT'),
            newSunset = moment.unix(sunset).utc().add(timezone_offset, 'seconds').format('LT'),
            dayDuration = moment.unix(sunset - sunrise).utc().add(timezone_offset, 'seconds').format('LT'),           
...
const Forecast = ({forecast}) => {
    ...
    return ( 
        ...
        daily.map((item, i) => {
            ...
            return (
                <Grid item xs={12} md={6} xl={3}>                    
                    <table key={i} className='table' style={{'background-color': 'gainsboro'}}>
                        <tr>                    
                            <td colspan="2" style={{'textAlign': 'center'}}>Прогноз погоды</td>
                        </tr>
                        <tr>                    
